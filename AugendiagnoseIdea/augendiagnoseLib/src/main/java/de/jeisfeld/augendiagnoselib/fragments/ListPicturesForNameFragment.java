	...
	public final boolean onContextItemSelected(@NonNull final MenuItem item) {
		if (item.getGroupId() == R.id.menugroup_picture_date) {
			...
			if (itemId == R.id.action_change_date) {
				...
				// delete images
				boolean success = pairToModify.moveToFolder(PreferenceUtil.getSharedPreferenceString(R.string.key_folder_input), true);
				// update list of images
				updateEyePhotoPairs();

				if (success) {
					if (getResources().getBoolean(R.bool.flag_go_to_organize_after_move_to_input)) {
